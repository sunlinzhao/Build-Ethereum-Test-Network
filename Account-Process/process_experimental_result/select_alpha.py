...
def show_only_btc_mean(data):
    # 图例标签
    labels = ['Standard Deviation', 'Modularity Value (q)', 'Stability Value (f)']
    # 设置线条颜色
    colors = ['r', 'g', 'b']
    # 设置点标记
    markers = ['o', 's', '^']
    ...
    # 为图表添加标题
    ax.set_title('Normalized Comparison of the Mean Metrics of Sharding Results')
...
def show_only_btc(data):
    ...
    # y轴标签
    y_labels = ["Standard Deviation", "Modularity Value (q)", "Stability Value (f)"]
    # 定义标签
    labels = ["α=0.2", "α=0.3", "α=0.4", "α=0.5", "α=0.6", "α=0.7", "α=0.8"]
    ...
    fig.suptitle('Comparison of Sharding Results Metrics')
    # 调整子图布局
    fig.tight_layout()
