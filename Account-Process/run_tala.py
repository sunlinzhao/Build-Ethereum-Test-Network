...
def start_tala(index, n, a, b):
    ...
    for i, hedges in enumerate(data_list):
        ...
        # 确保目录存在，如果不存在则创建
        directory = r'result-alpha\{}-{}\{}\{}'.format(a, b, name[i], time_str + "_" + str(len(communities0)))
        # 创建目录
...
if __name__ == "__main__":
    ...
    alpha_list = [[0.2, 0.8], [0.3, 0.7], [0.4, 0.6], [0.5, 0.5], [0.6, 0.4], [0.7, 0.3], [0.8, 0.2]]
    for alpha in alpha_list:
        for i in range(3): # 循环数据集
            for n in num_list: # 循环超边数量
                while not start_tala(i, n, alpha[0], alpha[1]):
                    continue
