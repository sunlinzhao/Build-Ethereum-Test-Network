...
def run_first_phase(node_to_comm, adj_matrix, n, force_merge=False, T_list: list = [], T_txs: list = [], a: float = 1.0,
                    b: float = 0.0):
    ...
    while is_updated:
        ...
        for i, neighbors in enumerate(adj_matrix):
            ...
            if n and num_communities == n:
                break
            q = a * modularity(M, invert_node_to_comm(best_node_to_comm))
            f = b * flath(T_list, T_txs, invert_node_to_comm(best_node_to_comm))
            best_Q = q + f
            ...
            for j, weight in enumerate(neighbors):
                ...
                qq = a * modularity(M, invert_node_to_comm(candidate_node_to_comm))
                ff = b * flath(T_list, T_txs, invert_node_to_comm(candidate_node_to_comm))
                candidate_Q = qq + ff
...
def louvain_method(adj_matrix: np.ndarray, n: int = None, T_list: list = [], T_txs: list = [], a: float = 1.0,
                   b: float = 0.0) -> list:
    ...
    def update_frame(frame, partition, comm_aliases, recalculate_Q):
        ...
        if recalculate_Q:
            ppp = a * modularity(M, invert_node_to_comm(frame["C"]))
            fff = b * flath(T_list, T_txs, invert_node_to_comm(frame["C"]))
            frame["Q"] = ppp + fff
