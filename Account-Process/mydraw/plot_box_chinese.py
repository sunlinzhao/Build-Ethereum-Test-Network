...
# 设置坐标轴标签
plt.xticks([1, 2, 3], ['Bitcoin', 'Ethereum', 'Eosio'])
plt.ylabel('概率')
...
# plt.figure()
fig, ax = plt.subplots()
plt.loglog(x1, linestyle='-', color='#D9958F', linewidth=1.5)
plt.loglog(x2, linestyle='--', color='#7E649E', linewidth=1.5)
plt.loglog(x3, linestyle=':', color='#31859B', linewidth=1.5)
# plt.title("Degree distribution")
plt.xlabel("度")
plt.ylabel("频次")
plt.legend(['Bitcoin', 'Ethereum', 'Eosio'])
plt.show()

# 将两个列表转换为 DataFrame
df = pd.DataFrame(list(zip(d1, d2, d3)), columns=['Bitcoin', 'Ethereum', 'Eosio'])
...
sns.violinplot(data=df, bw_method=0.2, palette=["#D9958F", "#7E649E", "#31859B"])
# 绘制箱线图
# 设置异常点的颜色和大小
